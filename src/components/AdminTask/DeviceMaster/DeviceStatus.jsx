...
import AddDeviceStatus from './AddDeviceStatus'
...
const DeviceStatus = () => {
    ...
    const { refetch : deviceStatusFetch } = useQuery(['deviceStatusList', deviceStatusFilter],deviceStatusList.bind(this,deviceStatusFilter),{ onSuccess : (x)=> {
        setData(x?.items);
    ...
    const columns = [
       
      {
        ...
        },
        {
          accessorKey: 'Action',
          header: 'Action',
          Cell: ( {row} ) => <AddDeviceStatus deviceStatusFetch={deviceStatusFetch} modalName='edit' statusName={row?.original?.name} statusActive={row?.original?.isActive} ids={row?.original?.id} />,
          mantineTableHeadCellProps: {
            align: 'center',
          },
          mantineTableBodyCellProps: {
  ...
  return (
    <div  className=' flex flex-col  gap-2 py-2 px-2 justify-start items-start mt-1 h-screen ' >
        <Headers breadList={breadList} name='Device Status' />
        ...
        <div className="pb-4 px-8 ">
          <div className={`${toggle ? "pb-4" : "pb-0"}`}>
            ...
            >
                <AddDeviceStatus modalName='add' deviceStatusFetch={deviceStatusFetch} />
