...
import { contactTypeList } from '../../../utills/api/ContactType'
...
const ContactType = () => {
    ...
    const [ contactTypeFilter, setContactTypeFilter ] = useState({
      ...
      const { refetch : contactTypeFetch } = useQuery(['contactTypeList', contactTypeFilter],contactTypeList.bind(this,contactTypeFilter),{ onSuccess : (x)=> {
        setData(x?.items);
      ...
      const columns = [
        ...
        },
        {
          ...
          header: 'Action',
          Cell : ({row}) =>{
              return  <AddContactType contactTypeFetch={contactTypeFetch} modalName='edit' typeName={row?.original?.name} typeCode={row?.original?.code} typeActive={row?.original?.isActive} ids={row?.original?.id} />
  ...
  return (
    <div  className=' flex flex-col  gap-2 py-2 px-2 justify-start items-start mt-1 h-screen ' >
      <Headers breadList={breadList} name='Contact Types' />
        ...
        <div className="pb-4 px-8">
          <div className={`${toggle ? "pb-4" : "pb-0"}`}>
            ...
            >
                    <AddContactType modalName='add' contactTypeFetch={contactTypeFetch} />
