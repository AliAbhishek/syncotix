...
const MasterDesignation = () => {
  ...
  const { refetch: desigFetch } = useQuery(
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Action',
      Cell : ({row}) =>{
          return <AddDesignation modalName='edit' desigFetch={desigFetch} code={row?.original?.code} designationId={row?.original?.id} designationName={row?.original?.name} designationActive={row?.original?.isActive} />
  ...
  return (
    <div className=" flex flex-col  gap-2 py-2 px-2 justify-start items-start mt-1 h-screen ">
      ...
      <Headers breadList={breadList} name='Designations' />
      ...
      <div className="w-full py-4 rounded-lg dark:text-white dark:bg-[#263238] bg-white text-black">
        <div className="pb-4 px-8 ">
          <div className={`${toggle ? "pb-4" : "pb-0"}`}>
            ...
            >
              <AddDesignation modalName="add" desigFetch={desigFetch} />
...
export default MasterDesignation;
